...
def open_target_group(page: Page, group_name: str) -> None:
    ...
    def is_group_open() -> bool:
        ...
        # First try explicit title match on header node.
        direct = page.locator(f"header span[title='{group_name}']").first
        if direct.count() > 0 and direct.is_visible():
            return True

        # Then scan all visible headers and check text content.
        ...
        for i in range(count):
            ...
            if not h.is_visible():
                continue
            try:
                txt = normalize(h.inner_text(timeout=800))
            except Exception:
                continue
            if target in txt:
                return True
    ...
    # If already in target chat, skip search flow.
    if is_group_open():
    ...
    try:
        # Prefer direct fill so we don't fail on intermittent viewport click issues.
    ...
    # Click exact row from chat list when available.
    ...
    try:
        ...
        for i in range(min(headers.count(), 4)):
            h = headers.nth(i)
            if h.is_visible():
                observed.append(h.inner_text(timeout=500).strip().replace("\n", " | "))
